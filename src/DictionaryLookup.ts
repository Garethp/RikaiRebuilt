...
import {
  DictionaryWithDb,
  DictionaryWithIndexedDb,
  isForKanji,
  isForNames,
} from "./interfaces/DictionaryDefinition";
...
export default class DictionaryLookup {
  ...
  async updateDictionaries(dictionaries) {
    ...
    for (const dictionary of dictionaries) {
      ...
      if (dictionary.isNameDictionary === true)
        copy.db = new NameDictionary(copy.id);
  ...
  async search(word: string, noKanji: boolean = false): Promise<SearchResults> {
    ...
    do {
      ...
      if (!noKanji || !dictionary.isKanjiDictionary) {
        ...
        else if (isForNames(dictionary))
          entry = await dictionary.db.findNames(word, this.config.nameMax);
  ...
  async wordSearch(
    word: string,
    dictionary: DictionaryWithIndexedDb,
    max?: number
  ) {
  ...
  async kanjiSearch(character): Promise<KanjiResult> {
    ...
    if (!this.kanjiData) {
      this.kanjiData = await FileReader.read(
        `${resourcePrefix}resources/kanji.dat`
      )
    ...
    if (!this.radData)
      this.radData = await FileReader.read(
        `${resourcePrefix}resources/radicals.dat`
      )
        .then((response) => response.text())
    ...
    [onkun, nanori, bushumei] = [onkun, nanori, bushumei].map((input: string) =>
      input.replace(/\s+/g, "\u3001 ")
    );
    ...
    result.radicals = this.radData.filter(
      (line, index) =>
        index !== result.radicalNumber - 1 && line.indexOf(character) !== -1
    );
